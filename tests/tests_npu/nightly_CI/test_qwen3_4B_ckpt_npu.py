...
def default_checkpoint_dir(args):
    ...
    return os.path.join(os.environ.get("SLIME_TEST_WORK_ROOT", "/tmp"), f"{MODEL_NAME}_slime_{save_mode}_{args.save_optimizer}_save_{args.load_optimizer}_load")
...
def execute(mode: str = "", optimizer: str = "cpu", checkpoint_dir: str = ""):
    ...
    sglang_args = "--rollout-num-gpus-per-engine 2 --rollout-num-gpus 8 --sglang-mem-fraction-static 0.4 --sglang-cuda-graph-max-bs 16 "
