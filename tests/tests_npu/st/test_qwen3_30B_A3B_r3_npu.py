...
USE_NIXL_TRANSPORT = bool(int(os.environ.get("SLIME_TEST_USE_NIXL", "0")))
...
def execute():
    fp8_path = os.path.join(HF_WEIGHTS_ROOT, f"{MODEL_NAME}-FP8")
    if USE_FP8_ROLLOUT:
        if not os.path.isdir(fp8_path):
            raise FileNotFoundError(
                f"SLIME_TEST_USE_FP8_ROLLOUT=1 requires {fp8_path}"
            )
        ckpt_args = (
            f"--hf-checkpoint {fp8_path} "
    ...
    rollout_args = (
        ...
        f"{'--rollout-data-transport nixl ' if USE_NIXL_TRANSPORT else ''}"
    ...
    eval_args = (
        ...
        f"--eval-prompt-data aime24 {DATASETS_ROOT}/aime-2024/aime-2024.jsonl "
