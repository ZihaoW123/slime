...
- 1 prefill (``--disaggregation-mode prefill``, Ascend transfer backend)
- 1 decode  (``--disaggregation-mode decode``, Ascend transfer backend)
...
MODEL_NAME = "Qwen3-4B"
MODEL_TYPE = "qwen3-4B"
TORCH_DIST_CKPT = f"{MEGATRON_WEIGHTS_ROOT}/{MODEL_NAME}_torch_dist"
NUM_GPUS = 6
NUM_TRAIN_GPUS = 4
...
def _launch_sglang_server(
    *,
    ...
    env = os.environ.copy()
    env["ASCEND_RT_VISIBLE_DEVICES"] = ",".join(gpus)
...
def execute():
    ...
    os.environ["ASCEND_MF_STORE_URL"] = f"tcp://{external_host}:13210"
    os.environ.setdefault("ASCEND_MF_TRANSFER_PROTOCOL", "sdma")
    processes: list[subprocess.Popen] = []

    # Restrict ASCEND_RT_VISIBLE_DEVICES to training GPUs before Ray starts so
