...
Implements the same interface as slime.agent.sandbox.Sandbox so
...
Environment variables:
    DOCKER_SANDBOX_HOST   SSH target, e.g. m00876805@80.48.5.89  (REQUIRED)
...
_DOCKER_HOST = os.environ.get("DOCKER_SANDBOX_HOST", "m00876805@80.48.5.89")
...
# ---------------------------------------------------------------------------
# Minimal Sandbox base (mirrors slime.agent.sandbox.Sandbox interface)
# ---------------------------------------------------------------------------
class Sandbox:
    """Abstract interface — matches slime.agent.sandbox.Sandbox."""
...
class DockerSandbox(Sandbox):
    ...
    async def __aenter__(self) -> "DockerSandbox":
        ...
        def _start():
            ...
            slime_head_host = os.environ.get("SLIME_HEAD_HOST", "")
            no_proxy=os.environ.get("no_proxy", f"127.0.0.1,localhost,{slime_head_host}")
            NO_PROXY=os.environ.get("NO_PROXY", f"127.0.0.1,localhost,{slime_head_host}")
            http_proxy=os.environ.get("docker_http_proxy", "")
            https_proxy=os.environ.get("docker_https_proxy", "")
