...
class ExternalEngineInfo:
    ...
    def parallel_config(self) -> dict[str, int]:
        pp_size = int(
            self.server_info.get("pp_size")
            or self.server_info.get("pipeline_parallel_size")
            or 1
        )
        return {
            "tp_size": int(
            ...
            "pp_size": pp_size,
            "ep_size": int(
                self.server_info.get("ep_size")
                or self.server_info.get("expert_parallel_size")
                or 1
            ),
            "moe_dp_size": int(
                self.server_info.get("moe_dp_size")
                or self.server_info.get("moe_data_parallel_size")
                or 1
            ),
        }
